...
1. Collect CPU trace:
   - Use: collect.trace with pid=" + pid + @" profile=cpu-sampling duration=00:01:00
   - This captures CPU sampling data to identify hot methods

2. Collect performance counters:
   - Use: collect.counters with pid=" + pid + @" providers=[System.Runtime] duration=00:01:00
   - This captures CPU and thread metrics over time

3. Collect thread stacks:
   - Use: collect.stacks with pid=" + pid + @"
...
1. Collect GC dump:
   - Use: collect.gcdump with pid=" + pid + @"
...
3. Collect GC heap trace:
   - Use: collect.trace with pid=" + pid + @" profile=gc-heap duration=00:01:00
...
3. Collect thread counters:
   - Use: collect.counters with pid=" + pid + @" providers=[System.Runtime] duration=00:00:30
   - This captures thread metrics

4. After collection, analyze the results:
   - Use analyze_dump_sos with the dump artifact
   - Use session.analyze with the session ID
...
2. Collect baseline trace:
   - Use: collect.trace with pid=" + pid + @" duration=00:01:00
...
5. After collection, analyze the results:
   - Use artifact.summarize with the artifact IDs
   - Use session.analyze with the session ID
